...
class Searchbar extends Component{
    ...
    onInputChange = (event) => { this.setState({ searchPictureName: event.currentTarget.value.toLowerCase() }) };
    onFormSubmit = (event) => {
        event.preventDefault();
        if (this.state.searchPictureName.trim()===''){return toast('Введите имя картинки')}
